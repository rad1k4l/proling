    ...
    <script>
        ...
        let categories = new Vue({
            ...
            methods : {
                ...
                save: function () {
                    if(this.state !== null){
                        axios.post("{{ route("panel.languages.state") }}" , {
                            data : this.state
                        }).then(response => {
                ...
                add: function () {
                    modal.open(this.modalTemplate, function (submitted) {
                        return new Promise(resolve => {
                            axios.post("{{ route("panel.languages.create") }}", {  submitted  })
                                .then(response => {
                                    if(checkResponse(response)){
                                        window.location.reload();
                                    }else {
                ...
                edit: function (id) {
                    axios.post("{{ route("panel.languages.get") }}", { id : id}).then(response => {
                        let that = this;
                        ...
                        if (checkResponse(response)) {
                            ...
                            data.translations.forEach(function (value) {
                                template["name_" + value.locale].data = value.name;
                            });
                            template._title = "Dil Redaktə";
                            modal.open(template, function (submitted) {
                                return new Promise(resolve => {
                                    axios.post("{{ route("panel.languages.update") }}",{
                                        id : id ,
                                        submitted
                                    }).then(response => {
                                        if(checkResponse(response)){
                ...
                del : function  (id) {
                    ...
                    swal({
                        ...
                        if (willDelete) {
                            axios.post("{{ route("panel.languages.delete") }}",{
                                id : id
                            }).then(response =>{
                                if(checkResponse(response)) {
                                    window.location.reload();
