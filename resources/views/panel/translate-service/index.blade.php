                                ...
                                <div class="card-content" style="height: 500px">
                                   <div class="col s10">
                                       <div class="dd" id="categories">
                                           @include("panel.translate-service.list")
                                       </div>
                                   </div>
    ...
    <script>
        ...
        let categories = new Vue({
            ...
            methods : {
                ...
                save: function () {
                    if(this.state !== null){
                        axios.post("{{ route("panel.translate.service.state") }}" , {
                            data : this.state
                        }).then(response => {
                ...
                add: function () {
                    modal.open(this.modalTemplate, function (submitted) {
                        return new Promise(resolve => {
                            axios.post("{{ route("panel.translate.service.create") }}", {  submitted  })
                                .then(response => {
                ...
                edit: function (id) {
                    axios.post("{{ route("panel.translate.service.get") }}", { id : id}).then(response => {
                        let that = this;
                        ...
                        if (checkResponse(response)) {
                            ...
                            modal.open(template, function (submitted) {
                                return new Promise(resolve => {
                                    axios.post("{{ route("panel.translate.service.update") }}",{
                                        id : id ,
                                        submitted
                                    }).then(response => {
                                        if(checkResponse(response)){
                ...
                del : function  (id) {
                    ...
                    swal({
                        ...
                        if (willDelete) {
                            axios.post("{{ route("panel.translate.service.delete") }}",{
                                category_id : id
                            }).then(response =>{
                                if(response.status ===  200)
                                    if (response.data.status === "OK"){
